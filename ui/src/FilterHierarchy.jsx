...
function TreeNode({ node, depth = 0 }){
  const filters = useMemo(() => gatherFilters(node), [node])
  const prim = node.primitive ?? MainStore().primitive(node.node.id)
  ...
  // Identify data-source children (origin/alt_origin > 0 and import === 0)
  const children = Array.isArray(node.children) ? node.children : []
  const isDataSource = (n)=>{
  ...
  const dsChildren = children.filter(isDataSource)
  const normalChildren = children.filter(c=>!isDataSource(c))
  const incomingCount = children.reduce((a,c)=>a + (c?.count ?? 0), 0)
  ...
  return (
    <div className={clsx('pl-2', depth > 0 && 'border-l border-slate-200 ml-2')}>
      <div className="rounded-lg border border-slate-200 bg-slate-50 p-2">
        <button className="flex w-full items-center justify-between text-left" onClick={()=>setExpanded(!expanded)}>
          ...
          <div className="text-[11px] text-slate-500 ml-2 shrink-0 flex items-center gap-2">
            ...
            <span>out: <span className="font-semibold text-slate-700">{node.count}</span></span>
          </div>
        ...
        {expanded && (
          <div className="mt-2 space-y-2">
            ...
            {filters.length > 0 ? filters.map((f, i) => (<FilterRow key={i} f={f} />)) : (
              <div className="text-[11px] text-slate-500">No filters at this stage</div>
            )}
            {dsChildren.length > 0 && (
              <div className="flex flex-col gap-1">
                {dsChildren.map((ds, i) => (
                  <div key={i} className="inline-flex items-center gap-2 rounded-md bg-amber-50 px-2 py-1 text-[11px] text-amber-900 border border-amber-200">
                    ...
                    <span className="font-medium truncate">{ds.primitive?.title ?? ds.node?.title ?? `#${ds.primitive?.plainId ?? ''}`}</span>
                    <span className="text-amber-800">({ds.count})</span>
                  </div>
                ))}
              </div>
            )}
      ...
      {expanded && normalChildren.length > 0 && (
        <div className="mt-1 space-y-1">
          {normalChildren.map((c, idx) => (
            <TreeNode key={idx} node={c} depth={depth + 1} />
          ))}
        </div>
      )}
    </div>

                        ...
                        {showDetails && <>
                            <div className="p-2 ">
                                ...
                                <div className="py-2 text-gray-500 text-sm">
                                    <UIHelper.Panel title="Colors" narrow>
                                        {(() => {
                                            // Color palette selection for element primitives
                                            const isElement = frame.type === "element"
                                            const elementPage = isElement ? frame.origin : underlying
                                            const themeKey = elementPage?.renderConfig?.theme || "default";
                                                ...
                                                <div className="space-y-3">
                                                    ...
                                                    <div className="flex items-center space-x-2">
                                                        <Select
                                                            ...
                                                            size="sm"
                                                            label="Text color"
                                                            selectedKeys={currentText.startsWith("#") ? [] : [currentText]}
                                                            onSelectionChange={(v) => {
                                                                const val = Array.from(v)[0];
                                                                if (typeof val === 'string') {
                                                                    frame.setField("referenceParameters.text_color", val)
                                                                }
                                                            }}
                                                        >
                                                            {themePaletterComponents.map(n => (
                                                        ...
                                                        <input
                                                            ...
                                                            value={/^#([0-9a-f]{6}|[0-9a-f]{3})$/i.test(currentText ?? '') ? currentText : '#ffffff'}
                                                            onChange={(e) => frame.setField('referenceParameters.text_color', e.target.value)}
                                                        />
                                                    </div>

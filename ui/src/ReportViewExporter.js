...
export default function ReportViewExporter({primitive, ...props}){
    ...
    const targetList = ["669f45c26e1bd45cc906346b", "666be85605ff1c2b42bc00d4", "669f455e6e1bd45cc90631be", "66a0c1e7abb4c2893f73d839", 
    ...
    function renderView(d){
        ...
        if( d.field ){
            return {id: d.id, title: ()=>``, canChangeSize: true, canvasMargin: [2,2,2,2], items: (stageOptions)=>RenderPrimitiveAsKonva(d.primitive, {...stageOptions, ...renderOptions, config: "field", field: d.field})}
        }
    ...
    const [renderedSet] = useMemo(()=>{
        ...
        for( const d of fields){
            const id = `${primitive.id}-${d}`
            set.push({field: d, id, primitive: activeTarget})
        }

...
registerRenderer({ type: "categoryId", id: 109, configs: "summary_section" }, function renderFunc(primitive, options = {}) {
  ...
  const theme = options.theme
  const resolvedFontColor = themeColor(theme, "primary", "#334155")
    ...
    },
    {
      ...
      render: (ctx, node) => {
        ...
        content = content.map(line => {
                ...
                .join("\n");
        const color = themeColor(theme, "muted")
        const t = mkText({
            ...
            sections.forEach((d,i)=>{
                ...
                if( preview ){
                    ...
                    if( thisRenderOptions.size === "size"){
                        thisRenderOptions.tableWidth = previewWidth
                        thisRenderOptions.tableHeight = previewHeight
                    }
                    ...
                    const chart = renderDatatable({
                        ...
                        renderOptions: {
                            ...
                            headerFontSize: (thisRenderOptions.legend_size  ?? 8) * 1.25
                        },
                        id: primitive.id,
                        stageOptions:{
                    ...
                    message.attrs.refreshCallback = options.imageCallback
                    g.add(message)

                    const messageHeight = message.height()
                    const messageBottom = message.y() + messageHeight
                    const availableWidth = previewWidth
                    const availableHeight = previewHeight.height - messageBottom 

                    let animationSize = Math.min(availableWidth, availableHeight > 0 ? availableHeight : Math.min(previewWidth, previewHeight) * 0.7)
                    if (availableHeight <= 0) {
                        animationSize = Math.min(availableWidth, Math.max(animationSize, 128))
                    }
                    animationSize = Math.max(animationSize, 32)

                    const animationTop = messageBottom + 12
                    const placeholder = createProcessingPlaceholder(animationSize, {backgroundFill: themeColor(theme, "background")})
                    const animationGroup = placeholder.group
                    animationGroup.x(x + ((sectionWidth - animationSize) / 2))
                    animationGroup.y(animationTop)
                    g.add(animationGroup)
                    ...
                    if (options.amimCallback) {
                        g.attrs.hasAnimationNode = animationGroup._id

...
import { createProcessingPlaceholder, updateProcessingPlaceholder, configureProcessingPlaceholderAnimation, tickProcessingPlaceholder } from "./renderers/animations";
import { normalizeSectionLayoutSpec, buildLayoutPreset, applyLayoutPresetToSections } from "./sharedLayout";
const typeMaps = {}
const categoryMaps = {}
...
registerRenderer( {type: "default", configs: "datatable_grid"}, function renderFunc({table, cell, renderOptions, childRenderOptions, stageOptions, ...options}){
    ...
    const itemWidth = typeof(defaultWidth) == "string" && defaultWidth.endsWith("_fixed") ? parseInt(defaultWidth.slice(0,-6)) : renderOptions.width ?? config.itemSize ?? defaultWidth
    const itemHeight = renderOptions.height
    ...
    const bgRect = new Konva.Rect({
        ...
        width: width,
        name: "background",
        fill: options.includeBackground ? themeColor(theme, "cell") : undefined,
        hoverFill: themeColor(theme, "hoverFill"),
        hoverStroke: themeColor(theme, "hoverStroke")
    });
    g.add(bgRect);
    let x = config.padding[3];
    ...
    const maxY = Math.max(...columnYs);
    let newHeight = maxY + config.padding[2];

    if( options.cellHeight ){
        newHeight = options.cellHeight
    }
...
registerRenderer( {type: "default", configs: "datatable_ranking"}, ({table, cell, renderOptions, ...options})=>{
    const config = {width: 200, height: 200, itemSize: 30, itemPadding: [2,2,2,2], padding: [10,10,10,10], ...(options.renderConfig ?? {})}

    if( options.cellHeight ){
        config.height = options.cellHeight
    }else if( options.maxHeight ){
        config.height = options.maxHeight
    }

    let items = cell.items

    const defaultWidth = defaultWidthByCategory[items[0]?.referenceId + "_" + options.config] ?? defaultWidthByCategory[items[0]?.referenceId] ?? 200
    config.width = typeof(defaultWidth) == "string" && defaultWidth.endsWith("_fixed") ? parseInt(defaultWidth.slice(0,-6)) : renderOptions.width ?? defaultWidth
    ...
    let rankBy = renderOptions?.rank ?? "title" 
    ...
    for( const d of items ){
        const node = RenderPrimitiveAsKonva( d, {
            ...
            padding: config.itemPadding, 
            imageCallback: options.stageOptions.imageCallback,
            theme: options.theme
            })
        if( node ){
            g.add(node)
...
registerRenderer( {type: "categoryId", id: [29,78], configs: "ranking"}, (primitive, options = {})=>{
    ...
    const theme = options.theme ?? defaultTheme
    const resolvedFontFamily = theme?.fontFamily 
    const resovledBackground = themeColor(theme, 'background', '#ffffff')
    const mainColor = themeColor(theme, 'primary', '#000000', options.text_color)
    const dataColor = themeColor(theme, 'secondary', '#000000', options.text_color)
    ...
    if( g ){
        ...
        const title = new Konva.Text({
            ...
            fontFamily: resolvedFontFamily,
            fill: mainColor,
            placeholderFill: mixHex( mainColor, resovledBackground, 0.7),
        ...
        const amount = new Konva.Text({
            ...
            fontFamily: resolvedFontFamily,
            fill: dataColor,
            placeholderFill: mixHex( dataColor, resovledBackground, 0.7),
...
registerRenderer({ type: "categoryId", id: 109, configs: "summary_section" }, function renderFunc(primitive, options = {}) {
  ...
  const themeStyle = options.renderOptions.themeStyle

  const theme = options.theme
  const resolvedFontColor = themeColor(theme, themeStyle ?  `${themeStyle}Text` : "primary", themeColor( theme, "primary", "#334155"))
  const resovledBackground = themeColor(theme, themeStyle ??  "surface", themeColor( theme, "background"))
  ...
  if( theme.padding?.[themeStyle] ){
    config.padding = theme.padding[themeStyle]
  }
  
  const availableWidth = config.width - config.padding[1] - config.padding[3];
  const availableHeight = config.maxHeight != null ? config.maxHeight - config.padding[0] - config.padding[2] : undefined;
  ...
  const mkText = (attrs) => {
        return new CustomText({
            ...
            lineHeight: 1.3,
            fill: resolvedFontColor,
            placeholderFill: attrs.placeholderFill ?? placeholderFill,
  ...
  const RENDERERS = [
    ...
    },
    {
      ...
      render: (ctx, node) => {
        ...
        const color = themeColor(theme, "secondary")
    ...
    },
    {
      ...
      render: (ctx, node) => {
        ...
        const t = mkText({
          ...
          fill: color,
          text: content,
          width: ctx.availableWidth - (ctx.config.padding[3] * 4) - (ctx.config.padding[1]),
    ...
    },
    {
      ...
      render: (ctx, node) => {
        ...
        const spaceX = ctx.spaceY / 2
        let consumed = 0
        const itemHeight = 48 + (ctx.spaceY * 0.5) + (ctx.config.fontSize * 0.6) + ctx.spaceY 
        let idx = -1
        for (const d of names) {
            idx++
            if( ctx.config.itemCount > 0 && idx >= ctx.config.itemCount){
                break
            }
            if( x + companySizing + spaceX > availableWidth){
                x = ctx.config.padding[3]
                consumed += itemHeight
            }
          const logo = imageHelper(`/api/companyLogo?name=${d}`, {
            x, y: ctx.y + consumed,
            width: companySizing,
          ...
          const t = mkText({
            x,
            y: ctx.y + consumed + (companySizing / 2) + (ctx.spaceY * 0.5),
          ...
          ctx.group.add(t);
          x += companySizing + spaceX
        }
        consumed += itemHeight + ctx.config.padding[2] + ctx.spaceY

        // total height chunk
        return { heightConsumed: consumed };
      }
    },
    {
      name: "organizations",
  ...
  if (Array.isArray(data) && data.length) {
    ...
    if( options.sectionConfig?.segment_title?.show){
  ...
  const totalheight = Math.max(y + config.padding[2] - ctxBase.spaceY, config.padding[0] + config.padding[2])
...
registerRenderer( {type: "categoryId", id: 109, configs: "default"}, function renderFunc(primitive, options = {}){
    ...
    if( g ){
       ...
       text = text?.replaceAll('\\n','\n')
        ...
        let totalheight = Math.max(h + config.padding[0] + config.padding[2] + (config.showId ? idHeight : 0), options.height ?? 0)
...
export function renderPlaceholder(renderOptions = {}){
    ...
    const r = new Konva.Rect({
        ...
        width: usableWidth,
        height: usableHeight,
        stroke: themeColor(theme, 'placeholder', '#e2e2e2'),
...
registerRenderer( {type: "categoryId", id: 154, configs: "default"}, function renderFunc(primitive, options = {}){
     ...
     config.height = config.height ?? 640
    ...
    if( g ){
        ...
        let imageFit = "cover"
        const url = `/api/image/${primitive.id}` + (primitive.imageCount ? `?${primitive.imageCount}` : "")
        if( url ){
...
function baseImageWithText(primitive, options){
    ...
    const resolvedFontFamily = options.fontFamily ?? theme?.fontFamily ?? "Arial";
    const resolvedFontColor = themeColor(theme, 'primary', '#000000', options.text_color)
    const resovledBackground = themeColor(theme, 'surface', '#ffffff')
    const resovledPlaceholderFill = mixHex( resolvedFontColor, resovledBackground, 0.7)
    ...
    if( g ){
        ...
        if( config.showId ){
            const idText = new CustomText({
                ...
                fontSize: config.idSize,
                text: `${primitive.displayType} #${primitive.plainId}`,
                fill: themeColor(theme, "secondary"),
                placeholderFill: resovledPlaceholderFill,
                wrap: true,
                width: availableWidth,
            })
...
function createActionButton({
    ...
    const background = new Konva.Rect({
        ...
        fill: themeColor(theme, "accent", "#1f2937"),
        opacity: 0.92
    });
...
function renderBaseThemeElements( page, theme, config, options){
    return renderThemeElements( page, theme, config, options, false)
}
function renderTopThemeElements( page, theme, config, options){
    return renderThemeElements( page, theme, config, options, true)
}
function renderThemeElements( page, theme, config, options, top){
    const elements = (theme.elements ?? []).filter(d=>top ? d.top === top : !d.top)
    ...
    for(const d of elements ){
        ...
        let position = {
            ...
            "top-right": [config.width - width - offsetX, offsetY],
            "bottom-left": [offsetX, config.height - height - offsetY],
            "bottom-right": [config.width - width - offsetX, config.height - height - offsetY],
        }[d.position] ?? [0,0]
        let element
        switch( d.type ){
            case "image":
                let url = `/api/companyLogo?domain=co-created.com`
                switch(d.source){
                    case "workspace":
                        url = MainStore().workspace(page.workspaceId).logoUrl 
                        break
                    case "url":
                        url = d.url ?? url
                        break
                }
                element = imageHelper( url, {
                    x: position[0],
                    y: position[1],
                    width, 
                    height,
                ...
                break
            case "text":
                const color = themeColor(theme, d.color)
                element = new CustomText({
                    text: d.text,
                    align: d.align ?? "left",
                    fontStyle: d.fontStyle,
                    fill: color,
                    placeholderFill: mixHex( color, themeColor(theme, "background"), 0.7),
                    x: position[0],
                    y: position[1],
                    width, 
                    height,
...
const LAYOUT_COLUMN_ORDER = ["left", "center", "right", "sidebar_left", "sidebar_right"];

function buildOrderResolver(baseOrder) {
    const cache = new Map();
    let extraIndex = baseOrder.length;
    return (key) => {
        if (!cache.has(key)) {
            const idx = baseOrder.indexOf(key);
            cache.set(key, idx >= 0 ? idx : extraIndex++);
        }
        return cache.get(key);
    };
}

const resolveRowOrder = buildOrderResolver(LAYOUT_ROW_ORDER);
const resolveColumnOrder = buildOrderResolver(LAYOUT_COLUMN_ORDER);

function resolveRowKey(slot) {
    if (!slot) return null;
    if (slot.startsWith("top") || slot.startsWith("upper")) return "top";
    if (slot.startsWith("bottom") || slot === "bottom_full" || slot === "footer") return "bottom";
    if (slot === "center" || slot.startsWith("middle") || slot === "callout" || slot === "sidebar_left" || slot === "sidebar_right" || slot === "full_width" || slot === "full_page") {
        return "middle";
    }
    return null;
}

function resolveColumnKey(slot) {
    if (!slot) return null;
    if (slot.endsWith("left") || slot === "sidebar_left") return "left";
    if (slot.endsWith("right") || slot === "sidebar_right" || slot === "callout") return "right";
    if (slot.includes("center") || slot === "center") return "center";
    if (slot === "full_width" || slot === "full_page" || slot === "bottom_full") return "center";
    return null;
}

function computeStructuredSectionFrames(sections, layoutSpec, bounds) {
    const { width, height, startX, startY, spacerX, spacerY } = bounds;
    const hintsArray = Array.isArray(layoutSpec?.sections) ? layoutSpec.sections : null;
    const preset = buildLayoutPreset(layoutSpec, sections.length);

    const layoutInfos = sections.map((section, index) => {
        let layoutHint = section?.layout;
        if (!layoutHint && Array.isArray(hintsArray)) {
            const hintMatch = hintsArray.find((entry, hintIndex) => {
                if (!entry) return false;
                if (entry.section_id != null) {
                    return entry.section_id === section?.id;
                }
                if (entry.id != null) {
                ...
                return hintIndex === index;
            });
            if (hintMatch) {
                layoutHint = hintMatch.layout ?? hintMatch;
            }
        }
        if (!layoutHint && preset?.sectionLayouts) {
            const presetHint = preset.sectionLayouts.find((entry) => entry.index === index);
            if (presetHint) {
                layoutHint = presetHint.layout;
            }
        }

        const normalized = normalizeSectionLayoutSpec(layoutHint);
        const slot = normalized?.slot;
        let rowKey = resolveRowKey(slot);
        let columnKey = resolveColumnKey(slot);

        if (!columnKey && (normalized?.size === "full" || slot === "full_width" || slot === "full_page" || slot === "bottom_full")) {
            columnKey = "center";
        }
        if (!rowKey && normalized?.size === "full") {
            rowKey = "middle";
        }
    ...
    const hasStructured = layoutInfos.some((info) => info.rowKey || info.columnKey || info.size === "full");
    if (!hasStructured) {
        return null;
    }

    layoutInfos.forEach((info, index) => {
        if (!info.rowKey) {
            if (info.slot?.startsWith("top")) {
                info.rowKey = "top";
            } else if (info.slot?.startsWith("bottom")) {
                info.rowKey = "bottom";
            } else {
                info.rowKey = `auto_${index}`;
            }
        }
        if (!info.columnKey) {
            info.columnKey = "center";
        }
    });

    let columnsOrder = Array.from(new Set(layoutInfos.map((info) => info.columnKey)));
    if (columnsOrder.length === 1 && (columnsOrder[0] === "left" || columnsOrder[0] === "right")) {
        const replacement = "center";
        layoutInfos.forEach((info) => {
            if (info.columnKey === columnsOrder[0]) {
                info.columnKey = replacement;
            }
        });
        columnsOrder = [replacement];
    }
    columnsOrder.sort((a, b) => resolveColumnOrder(a) - resolveColumnOrder(b));

    const rowsOrder = Array.from(new Set(layoutInfos.map((info) => info.rowKey)));
    rowsOrder.sort((a, b) => resolveRowOrder(a) - resolveRowOrder(b));

    const totalCols = Math.max(columnsOrder.length, 1);
    let totalRows = Math.max(rowsOrder.length, 1);

    const columnIndexMap = new Map();
    columnsOrder.forEach((col, idx) => columnIndexMap.set(col, idx));
    const rowIndexMap = new Map();
    rowsOrder.forEach((row, idx) => rowIndexMap.set(row, idx));

    layoutInfos.forEach((info) => {
        info.columnIndex = columnIndexMap.get(info.columnKey) ?? 0;
        info.rowIndex = rowIndexMap.get(info.rowKey) ?? 0;
    });

    const columnNormalizationMap = new Map();
    layoutInfos.forEach((info) => {
        const list = columnNormalizationMap.get(info.columnIndex) ?? [];
        list.push(info);
        columnNormalizationMap.set(info.columnIndex, list);
    });
    columnNormalizationMap.forEach((list) => {
        const minRowIndex = Math.min(...list.map((entry) => entry.rowIndex ?? 0));
        if (Number.isFinite(minRowIndex) && minRowIndex > 0) {
            list.forEach((entry) => {
                entry.rowIndex -= minRowIndex;
            });
        }
    });

    const uniqueRowIndices = Array.from(new Set(layoutInfos.map((entry) => entry.rowIndex ?? 0))).sort((a, b) => a - b);
    if (uniqueRowIndices.length > 0) {
        const compressionMap = new Map();
        uniqueRowIndices.forEach((value, idx) => compressionMap.set(value, idx));
        layoutInfos.forEach((info) => {
            const compressed = compressionMap.get(info.rowIndex);
            if (compressed != null) {
                info.rowIndex = compressed;
            }
        });
        totalRows = Math.max(uniqueRowIndices.length, 1);
    }

    const byColumn = new Map();
    layoutInfos.forEach((info) => {
        const list = byColumn.get(info.columnIndex) ?? [];
        list.push(info);
        byColumn.set(info.columnIndex, list);
    });
    byColumn.forEach((list) => list.sort((a, b) => a.rowIndex - b.rowIndex));

    layoutInfos.forEach((info) => {
        const columnEntries = byColumn.get(info.columnIndex) ?? [];
        const selfIndex = columnEntries.indexOf(info);
        let nextRowIndex = totalRows;
        if (selfIndex !== -1 && selfIndex + 1 < columnEntries.length) {
            nextRowIndex = columnEntries[selfIndex + 1].rowIndex;
        }
        info.rowSpan = Math.max(1, nextRowIndex - info.rowIndex);
    });

    layoutInfos.forEach((info) => {
        let colSpan = 1;
        const slot = info.slot;
        const isCentralFullSlot = slot === "full_width" || slot === "full_page" || slot === "bottom_full" || slot === "top_center" || slot === "bottom_center" || slot === "center";
        const isEdgeSlot = slot?.endsWith("left") || slot?.endsWith("right") || slot === "sidebar_left" || slot === "sidebar_right";

        if (info.size === "full" && isCentralFullSlot && totalCols > 1) {
            colSpan = totalCols;
            info.columnIndex = 0;
        } else if (info.size === "half" && totalCols > 1 && !isEdgeSlot) {
            colSpan = Math.min(2, totalCols);
            if (info.columnIndex + colSpan > totalCols) {
                info.columnIndex = totalCols - colSpan;
            }
        } else if (info.size === "full" && !isEdgeSlot && totalCols > 1 && columnIndexMap.size === 1) {
            colSpan = Math.min(totalCols, 2);
        }
        info.colSpan = colSpan;
        info.initialColumnIndex = info.columnIndex ?? 0;
    });

    const occupancy = Array.from({ length: totalRows }, () => Array(totalCols).fill(null));

    const fillOccupancy = (info) => {
        const rowSpan = Math.max(1, info.rowSpan ?? 1);
        const colSpan = Math.max(1, info.colSpan ?? 1);
        for (let r = info.rowIndex; r < info.rowIndex + rowSpan; r += 1) {
            for (let c = info.columnIndex; c < info.columnIndex + colSpan; c += 1) {
    ...
    layoutInfos.forEach(fillOccupancy);
    const baseOccupancy = occupancy.map((row) => [...row]);

    const hasBaseOccupantToLeft = (info) => {
        const startColumn = info.initialColumnIndex ?? info.columnIndex ?? 0;
        if (startColumn === 0) {
            return false;
        }
        const rowSpan = Math.max(1, info.rowSpan ?? 1);
        for (let r = info.rowIndex; r < info.rowIndex + rowSpan; r += 1) {
            const row = baseOccupancy[r];
            if (!row) continue;
            for (let c = 0; c < startColumn; c += 1) {
                const occupant = row[c];
    ...
    const columnAvailable = (info, column) => {
        const rowSpan = Math.max(1, info.rowSpan ?? 1);
        for (let r = info.rowIndex; r < info.rowIndex + rowSpan; r += 1) {
            const occupant = occupancy[r]?.[column];
            if (occupant && occupant !== info) {
                return false;
            }
        }
    ...
    const sortedInfos = [...layoutInfos].sort((a, b) => {
    ...
    sortedInfos.forEach((info) => {
        const rowSpan = Math.max(1, info.rowSpan ?? 1);
        // expand right
        while (info.columnIndex + info.colSpan < totalCols) {
            const targetCol = info.columnIndex + info.colSpan;
            if (!columnAvailable(info, targetCol)) {
                break;
            }
            for (let r = info.rowIndex; r < info.rowIndex + rowSpan; r += 1) {
                occupancy[r][targetCol] = info;
            }
            info.colSpan += 1;
        }

        // expand left
        if (!hasBaseOccupantToLeft(info)) {
            while (info.columnIndex > 0) {
                const targetCol = info.columnIndex - 1;
                if (!columnAvailable(info, targetCol)) {
                    break;
                }
                info.columnIndex -= 1;
    ...
    const columnWidth = (width - spacerX * (totalCols - 1)) / totalCols;
    const rowHeight = (height - spacerY * (totalRows - 1)) / totalRows;

    return sections.map((section, index) => {
        const info = layoutInfos.find((entry) => entry.index === index);
        if (!info) {
            return null;
        }
        const colIndex = info.columnIndex ?? 0;
        const rowIndex = info.rowIndex ?? 0;
        const colSpan = Math.max(1, info.colSpan ?? 1);
        const rowSpan = Math.max(1, info.rowSpan ?? 1);

        const x = startX + colIndex * (columnWidth + spacerX);
        const y = startY + rowIndex * (rowHeight + spacerY);
        const frameWidth = columnWidth * colSpan + spacerX * (colSpan - 1);
        const frameHeight = rowHeight * rowSpan + spacerY * (rowSpan - 1);
...
function computeSequentialSectionFrames(count, bounds) {
    const { width, height, startX, startY, spacerX } = bounds;
    if (count === 0) {
        return [];
    }
    const sectionWidth = (width - spacerX * (count - 1)) / count;
    return Array.from({ length: count }, (_, index) => ({
        x: startX + index * (sectionWidth + spacerX),
        y: startY,
        width: sectionWidth,
        height: height,
    }));
...
function renderProcessingState({ group, x, y, width, height, spacerY, theme, id, options, messageText = "Working..." }) {
    const messageWidth = Math.min(width * 0.85, 520);
    const messageX = x + (width - messageWidth) / 2;
    const messageY = y + spacerY;
    const message = new CustomText({
        x: messageX,
        y: messageY,
        width: messageWidth,
        text: messageText,
    ...
    const messageBottom = message.y() + message.height();
    const availableHeight = height - (messageBottom - y) - spacerY;
    let animationSize = Math.min(width * 0.6, availableHeight > 0 ? availableHeight : width * 0.6);
    animationSize = Math.max(64, Math.min(animationSize, 280));
    const animationTop = messageBottom + spacerY;

    const placeholder = createProcessingPlaceholder(animationSize, {
        backgroundFill: themeColor(theme, "background"),
        animationId: id,
    });
    const animationGroup = placeholder.group;
    animationGroup.x(x + (width - animationSize) / 2);
    animationGroup.y(animationTop);
    group.add(animationGroup);

    const chartOrder = ["bars", "area", "table", "bars", "textbox", "pie"];
    configureProcessingPlaceholderAnimation(placeholder, {
        chartOrder,
        transitionDuration: 600,
        pauseDuration: 800 * (1 + Math.floor(Math.random() * 4) / 4),
        reset: true,
    });


    updateProcessingPlaceholder(placeholder, {
        chartKey: chartOrder[0],
        stage: "build",
        stageProgress: 0,
        shimmerProgress: 0,
        hue: 0,
    ...
    if (options.amimCallback) {
        group.attrs.hasAnimationNode = animationGroup._id;
        options.amimCallback(animationGroup, (tick) => tickProcessingPlaceholder(placeholder, tick));
    }
}
 ...
 function renderSlideSuggestions(primitive, g, theme, options){
            ...
            const sectionsInput = skeleton.sections ?? [];
            const appliedSkeletonLayout = applyLayoutPresetToSections(skeleton.layout, sectionsInput);
            const sections = appliedSkeletonLayout.sections ?? sectionsInput;
            const layoutForFrames = appliedSkeletonLayout.layout ?? skeleton.layout;
            const mainstore = MainStore();

            const contentTop = margin[0] + (headerFontSize * 2 * 1.05) + spacer[1];
            const contentHeight = Math.max(0, 1080 - margin[2] - contentTop);

            const layoutFrames = computeStructuredSectionFrames(sections, layoutForFrames, {
                width: mainWidth,
                ...
                spacerX: spacer[1],
                spacerY: spacer[0]
            }) ?? computeSequentialSectionFrames(sections.length, {
                width: mainWidth,
                height: contentHeight,
                startX: margin[3],
                startY: contentTop,
            ...
            sections.forEach((d, i) => {
                ...
                const { x, y, width: sectionWidth, height: sectionHeight } = frame;
                const sectionData = options.data?.sections?.[i];
                const summaryPayload = d.materialized ?? {};
                const legacyPayload = primitive.referenceParameters?.materializedSummaries;
                const legacySummary = typeof legacyPayload?.summaryText === "string" ? legacyPayload.summaryText : "";
                const rawSummary = typeof summaryPayload.plain === "string" ? summaryPayload.plain : legacySummary;
                const summaryText = rawSummary.replace(/\s*\[\[id:[^\]]+\]\]/g, "").trim();
                const isPending = d?.runtime_status === "pending" || d?.status === "pending";
                ...
                const displayTitle = typeof d.title === "string" && d.title.trim().length > 0
                    ? d.title.trim()
                    : "Preview not available";

                const textNode = new Konva.CustomText({
                    text: displayTitle,
                    fontSize: 26,
                    align: "center",
                    x: x + 32,
                    width: Math.max(sectionWidth - 64, 120),
                    wrap: true,
                    fontFamily: theme.fontFamily,
                    fill: themeColor(theme, "primary"),
                });

                if (d.type === "summary") {
                    const contentPadding = 24;
                    const buttonHeight = 44;
                    const buttonWidth = Math.min(220, sectionWidth - contentPadding * 2);
                    const buttonX = x + (sectionWidth - buttonWidth) / 2;
                    const buttonY = y + sectionHeight - buttonHeight - spacer[0];

                    const summaryAreaX = x + contentPadding;
                    const summaryAreaY = y + contentPadding;
                    const summaryAreaWidth = sectionWidth - contentPadding * 2;
                    const summaryAreaHeight = Math.max(buttonY - summaryAreaY - spacer[0], 80);

                    const hasSummary = summaryText.length > 0;

                    const summaryNode = new CustomText({
                        x: summaryAreaX,
                        y: summaryAreaY,
                        width: summaryAreaWidth,
                        height: summaryAreaHeight,
                        text: hasSummary ? summaryText : "Summary not generated yet.",
                        align: "left",
                        wrap: true,
                        lineHeight: 1.35,
                        fontSize: hasSummary ? 22 : 20,
                        fill: themeColor(theme, hasSummary ? "primary" : "muted"),
                        fontFamily: theme.fontFamily,
                        withMarkdown: true,
                        refreshCallback: options.imageCallback,
                    });
                    ...
                    const buttonLabel = hasSummary ? "Refresh summary" : "Generate summary";
                    const button = createActionButton({
                        ...
                        imageCallback: options.imageCallback,
                        onClick: async () => {
                            await mainstore.doPrimitiveAction(primitive, "run_agent_materialize_summary", {
                                sectionId: d.id,
                            });
                        },
                    });
                    ...
                    if (displayTitle) {
                        textNode.y(Math.max(y + spacer[0], buttonY - textNode.height() - spacer[0]));
                        g.add(textNode);
                    }
                    return;
                }
                ...
                if (preview) {
                    const previewWidth = Math.min(sectionWidth * 0.75, sectionWidth - 64);
                    const previewHeight = Math.min(sectionHeight * 0.6, sectionHeight - textNode.height() - spacer[0] * 3);
                    const previewX = x + (sectionWidth - previewWidth) / 2;
                    const previewY = y + spacer[0];

                    const renderOptions = preview.renderOptions ?? {};
                    if (renderOptions.size === "size") {
                        renderOptions.tableWidth = previewWidth;
                        renderOptions.tableHeight = previewHeight;
                    }
                    ...
                    const chart = renderDatatable({
                        ...
                        renderOptions: {
                            ...
                            headerFontSize: (renderOptions.legend_size ?? 8) * 1.25,
                        },
                        id: primitive.id,
                        stageOptions: {
                            x: previewX,
                    ...
                    if (displayTitle) {
                        textNode.y(Math.min(y + sectionHeight - textNode.height() - spacer[0], previewY + previewHeight + spacer[0]));
                        g.add(textNode);
                    }
                    return;
                }
                ...
                renderProcessingState({
                    ...
                    theme,
                    options,
                    messageText: "Awaiting generation...",
                });
                if (displayTitle) {
                    textNode.y(y + sectionHeight - textNode.height() - spacer[0]);
                    g.add(textNode);
                }
            });
...
registerRenderer( {type: "type", id: "page", configs: "default"}, (primitive, options = {})=>{
    const config = {
        ...options,
        width: 1920 ?? options.width, 
        height: 1080 ?? options.height, 
        pageColumns: 6 ?? options.pageColumns 
    }
    ...
    if( !options.utils?.prepareBoards){
        ...
        const subrenders = subboards.map(d=>options.utils.renderBoard(d, {imageCallback: options.imageCallback, amimCallback: options.amimCallback, theme }))
            let i = 0
            ...
            if( theme.elements ){
                ...
                elements.attrs.pageTrack = pageIdx
                sg.add( elements )
            }
            ...
            if( theme.elements ){
                const elements = renderTopThemeElements(primitive, theme, config, options) 
                elements.attrs.pageTrack = pageIdx
                sg.add( elements )
            }
...
registerRenderer( {type: "categoryId", id: 29, configs: "default"}, (primitive, options = {})=>{
    const config = {width: 80, padding: [10,10,10,10], ...options}
    ...
    const backgroundColor = themeColor(options.theme, "background", "#ffffff")
...
function renderSubCategoryChart( title, data, options = {}){
    ...
    if( !colors ){
        colors = options.theme?.palette?.categoryColors ?? categoryColors
        if( options.colorMap ){
...
registerRenderer( {type: "default", configs: "datatable_distribution"}, function renderFunc({table, cell, renderOptions, ...options}){
    ...
    const sg = renderSubCategoryChart("", values, {
        ...
        colors: renderOptions.colorMap ? Object.values(renderOptions.colorMap) : undefined,
...
registerRenderer( {type: "default", configs: "datatable_timeseries"}, function renderFunc({table, cell, renderOptions, ...options}){
    ...
    for( const series of timeseries ?? [] ){
        ...
        const dx = config.width / Math.max(1, max - min)        
...
export function renderDatatable({id, primitive, data, stageOptions, renderOptions, viewConfig, ...options}){
    ...
    function buildLegend(){
        ...
        if( !legendInfo ){
            ...
            legendInfo = {
                colors,
                data: valuesForLegend,
                colorMap: valuesForLegend.reduce((a,d, i)=>{
                    a[d.label] = colors[i]
                    return a
                }, {})
            }
        }
        legend = renderLegend( legendInfo.data,{
                            ...options,
                            fontSize: legendFontSize,
                            horizontalLegend: !showOnRight,
                            width: showOnRight ? 200 : renderOptions.tableWidth,
                            height: showOnRight ? renderOptions.tableHeight : 300,
    ...
    const buildLegendFirst = (renderOptions.tableHeight || renderOptions.tableWidth) && showSingleLegend

    if( buildLegendFirst ){
        legend = buildLegend()
    }
    if( renderOptions.tableWidth ){
        baseCellConfig.width = renderOptions.tableWidth / columns.length
    }
    if( renderOptions.tableHeight ){
        let chartHeight = renderOptions.tableHeight

        if( legend && showSingleLegend  && legendPosition === "below"){
            chartHeight -= legend.height()
            chartHeight -= legendFontSize * 1.5
        }
        if( showColumnheaders ){
            chartHeight -= (renderOptions.headerFontSize ?? 10) * 1.5
        }
        
        baseCellConfig.height = chartHeight / rows.length
    }



    relayConfig.width = baseCellConfig.width
    relayConfig.height = baseCellConfig.height
    ...
    const childRenderOptions = {
        themeStyle: renderOptions.themeStyle
    }
    ...
    if(data.cells.length === 1 ){
        childRenderOptions.width = renderOptions.width ?? childRenderOptions.width
        childRenderOptions.height = renderOptions.height ?? childRenderOptions.height
    }
    ...
    const headerBackground = renderOptions.headerBackground
    const headers = prepareHeaders({columns: showColumnheaders ? columns : [], rows: showRowheaders ? rows : [], columnWidths, rowHeights, columnX, rowY, spacing, background: headerBackground, renderOptions, theme, refreshCallback: imageCallback})
    let footers
    ...
    if( legendPosition && showSingleLegend ){
        ...
        if( showOnRight ){
            ...
            legend.y( maxY + legendFontSize * 1.25)
...
function prepareHeaders({columns, rows, columnWidths, rowHeights, columnX, rowY, renderOptions, baseFontSize = 12, spacing, textPadding = [5,5,5,5], refreshCallback, includeColumns = true, includeRows = true, background = true, theme = defaultTheme,...options}){
    ...
    const maxRowHeight = Math.min(Math.max(...rowHeights), 200)
    let maxFont =  Math.max(6, maxRowHeight / 4)
    let headerScale = Math.max(1, Math.max(maxColumnWidth / 200 , maxRowHeight / 45 ))
    let headerFontSize = renderOptions.headerFontSize ?? options.fontSize ?? Math.min(baseFontSize * headerScale, maxFont, baseFontSize * 10)

...
export default function MyCombo({selectedItem, setSelectedItem, ...props}) {
  ...
  return (
    <Combobox multiple={Array.isArray(selectedItemReal)} as="div" value={selectedItemReal} onChange={setSelectedItemReal}>
      ...
      <div className="relative w-96">
        <Combobox.Input
          ...
          displayValue={(item) => Array.isArray(item) ?  `${props.prefix}${item.length} items` : props.items ? `${props.prefix}${props.items[item]?.title}` : ""}
        />

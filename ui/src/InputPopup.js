        ...
        <Popup width='max-w-xl' setOpen={closeModal} title={`${props.title || "Input needed"}`}>
            {({ activeOption }) => (
                ...
                {Object.keys(data).map((k)=>{
                    ...
                    <PrimitiveCard.RenderItem editable local {...data[k]} callback={(d)=>updateValue(k,d)} item={data[k]} primitive={props.primitive} ensurePresent types={data[k].primitiveTypes} referenceIds={data[k].referenceIds}/>
                    </>
                })}
                </div>
